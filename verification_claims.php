...
switch ($sort_by) {
    case 'high_cost':
        $sql = "SELECT claim_id, policy_id, user_id, claim_status, verification_status, filing_date, claim_amount, claim_category FROM claims WHERE verification_status = 'pending' AND claim_status = 'pending' ORDER BY claim_amount DESC";
        break;
    case 'low_cost':
        $sql = "SELECT claim_id, policy_id, user_id, claim_status, verification_status, filing_date, claim_amount, claim_category FROM claims WHERE verification_status = 'pending' AND claim_status = 'pending' ORDER BY claim_amount ASC";
        break;
    case 'recent':
    default:
        $sql = "SELECT claim_id, policy_id, user_id, claim_status, verification_status, filing_date, claim_amount, claim_category FROM claims WHERE verification_status = 'pending' AND claim_status = 'pending' ORDER BY filing_date DESC";
        break;
}
...
<body>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><a href="verify_claims.php?id=<?= $row['claim_id'] ?>" class="action-btn">Review</a></td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
    ...
    <script>
        function applySorting() {
            ...
            window.location.href = `verification_claims.php?sort_by=${sortBy}`;
        }
    </script>
</body>
</html>
