...
// Construct SQL query based on sort parameter - modified to only show approved or rejected claims
switch ($sort_by) {
    case 'high_cost':
        $sql = "SELECT claim_id, policy_id, user_id, claim_status, filing_date, claim_amount, claim_category FROM claims WHERE claim_status IN ('approved', 'rejected') ORDER BY claim_amount DESC";
        break;
    case 'low_cost':
        $sql = "SELECT claim_id, policy_id, user_id, claim_status, filing_date, claim_amount, claim_category FROM claims WHERE claim_status IN ('approved', 'rejected') ORDER BY claim_amount ASC";
        break;
    case 'recent':
    default:
        $sql = "SELECT claim_id, policy_id, user_id, claim_status, filing_date, claim_amount, claim_category FROM claims WHERE claim_status IN ('approved', 'rejected') ORDER BY filing_date DESC";
        break;
}
...
<head>
    ...
    <title>Completed Claims - Staff</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="container">
            <h2>Completed Claims Dashboard</h2>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            ?>
                            <td class="<?= $status_class ?>"><?= htmlspecialchars($row['claim_status']) ?></td>
                            ...
                            <td><a href="view_claim_details.php?id=<?= $row['claim_id'] ?>" class="action-btn">Details</a></td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="8">No completed claims found.</td></tr>
                <?php endif; ?>
            </table>
        </div>
    </div>
    ...
    <script>
        function applySorting() {
            ...
            window.location.href = `completed_claims.php?sort_by=${sortBy}`;
        }
    </script>
</body>
</html>
